...
- `spacy` — tokenizer + lemmatizer. The exact `en-core-web-sm==3.8.0` wheel is a direct-URL/hash requirement in every supported platform root lock; install the selected lock through `make install-torch-stack` rather than invoking spaCy's downloader.
...
- **`en_core_web_sm` is separately packaged but lock-owned.** Installing unqualified `spacy` alone does not supply the English model. The supported installer consumes the exact URL/hash entry from the selected platform root lock; later package-changing downloader commands are prohibited.
