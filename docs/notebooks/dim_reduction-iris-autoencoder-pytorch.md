...
- **Persist and reload the AE via `NNCheckpoint.load(run=RUN_ID, type=Checkpoints.BEST)`.** The notebook uses the live `run`
  object because the AE trains in seconds; a follow-up that saves the AE to `./runs/` and
  reloads its best checkpoint would exercise the serialization contract that the
