...
fresh session, which is how a downstream evaluation or deployment script would reload a trained
model without re-running the training loop. In this notebook the live `run`
...
- **Persist and reload via `NNCheckpoint.load(run=..., type=Checkpoints.BEST)`.** The notebook
  uses the live `run` object because the candidates train in seconds; a follow-up that saves each
  run to `./runs/` and reloads its best checkpoint exercises the serialization contract
